<template>
    <div class="todoList">
      <input type="text" v-model="newItems" v-on:keyup.enter="addItem" class="inputText"><button @click="addItem" class="todo-bnt">add</button>
      <!--<ul class="todo-ul">-->
        <transition-group tag="ul" name="fade" mode="in-out" class="todo-ul">
          <li v-for="(item,index) in items" v-bind:class="{Finished:item.isFinished}" :key="index">
            ...
            <div class="delete" @click.pevent="deleteItem(index)">X</div>
          </li>
        </transition-group>
      <!--</ul>-->
...
<style scoped>
  .Finished{text-decoration: line-through;color: #ffae22 !important;}
  ...
  .delete{display: inline-block;float:right;color: #ff313e;cursor:pointer;
