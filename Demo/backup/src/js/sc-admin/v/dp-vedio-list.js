...
define(function(require, exports, module) {
    ...
    $(function() {
        ...
        // 左侧导航选中
        $('.mod-sidenav').children('dd').eq(0).find('dd').eq(3).find('a').toggleClass('active');
       ...
       //审核通过
        ...
        // 单项删除
        $('.tb-vedio-list').on('click','.lnk-detele',function(){
            setSelectArr('chk', 1, this);
            deleteConfirm('您确定删除该项吗？');
        });

        // 播放
        $('.tb-vedio-list').on('click','.btn-play',function(){
            var $pop = $('#popPlayVedio');
            $pop.popup({
                "esc": true
            });
        });
